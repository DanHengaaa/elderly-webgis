      ...
      <section class="profile-grid">
        ...
        <div class="info-card">
          ...
          <div class="info-list">
            <div class="info-item">
              <span>用户名</span>
              <strong>{{ profile.username || '未填写' }}</strong>
            </div>

            <div class="info-item">
              <span>昵称</span>
              <strong>{{ profile.nickname || '未填写' }}</strong>
            </div>
            ...
            <div
              v-if="shouldShowInstitution"
              class="info-item"
            >
              <span>绑定机构</span>
              <strong>{{ institutionDisplayName }}</strong>
            </div>
          </div>
        </div>
...
const avatarText = computed(() => {
  const name = profile.nickname || profile.username || '养'
  return String(name).slice(0, 1)
})

const isCustomer = computed(() => {
  return profile.roleCode === 'CUSTOMER'
})

const shouldShowInstitution = computed(() => {
  return profile.roleCode === 'INSTITUTION'
})

const institutionDisplayName = computed(() => {
...
.profile-page {
  min-height: 100vh;
  background:
    radial-gradient(circle at top left, rgba(46, 125, 107, 0.18), transparent 32%),
    linear-gradient(180deg, #f4faf8 0%, #eef5f2 100%);
...
.profile-header {
  ...
  box-shadow: 0 16px 40px rgba(30, 80, 70, 0.12);
...
.profile-hero {
  ...
  border-radius: 30px;
  background:
    linear-gradient(135deg, rgba(46, 125, 107, 0.96), rgba(103, 198, 163, 0.9));
  box-shadow: 0 18px 45px rgba(46, 125, 107, 0.22);
  display: flex;
  align-items: center;
  gap: 22px;
  color: #fff;
...
.form-card {
  ...
  padding: 26px;
  border: 1px solid rgba(46, 125, 107, 0.1);
  box-shadow: 0 18px 45px rgba(30, 80, 70, 0.09);
}
